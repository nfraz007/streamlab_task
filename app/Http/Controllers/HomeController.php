...
class HomeController extends Controller
{
    public function dashboard(Request $request) {
        ...
        // $data["stream_per_game"] = $streams->groupBy("game")->map->count();
        $data["stream_per_game"] = $streams->groupBy("game")->map->count();
        $data["highest_viewer_per_game"] = $streams->groupBy("game")->map->max("avg_viewers");
        $data["median_viewer_per_game"] = $streams->groupBy("game")->map->median("avg_viewers");
        $data["odd_streams"] = $streams->filter(function($v) {
            return $v->streamers % 2 == 1;
        })->values();
        $data["even_streams"] = $streams->filter(function($v) {
            return $v->streamers % 2 == 0;
        })->values();
        $data["top_100_streams"] = $streams->sortByDesc("streamers")->take(100)->values();
        $data["stream_same_viewer"] = $streams->filter(function($v) {
            return $v->streamers == $v->avg_viewers;
        })->values();
